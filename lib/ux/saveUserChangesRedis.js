...
module.exports=function(socket,RedisClientUserDB){
  ...
  RedisClientUserDB.HMGET(query, function(err, result) {
    if (err) {
      ...
      if(socket.user_changes.new_subscriptions.length>0){
        changedQuery.push('subscribed_public_channels');
        var new_subscribed_channels_list=[];
        if(result[3]===''){ //if the array is empty (saves as an empty string in redis instead of an [], so JSON.parse doesn't work)
          new_subscribed_channels_list=socket.user_changes.new_subscriptions;
        }
        else{
          new_subscribed_channels_list=JSON.parse(result[3]).concat(socket.user_changes.new_subscriptions);
        }
        changedQuery.push(JSON.stringify(new_subscribed_channels_list));
      }
