...
var getUserMongo=require('./getUserMongo');
var saveUserChangesRedis=require('./../ux/saveUserChangesRedis');
...
module.exports = function(http, RedisClient) {
    ...
    if (sessionID) {
      ...
      RedisClient.SessionDB.get("sessions:" + sessionID, function(err, session) {
        ...
        if (err) { //if an error occurred retrieving the session from the db
          ...
          //TODO STOP GIVING USER THE ID AS THE USER DOES NOT NEED IT FOR ANY CLIENT SIDE FUNCTIONS
          var query=[
            'user:'+username,
            '_id',
            'username',
          ...
          RedisClient.UserDB.HMGET(query, function(err, result) {
            ...
            else if (!result[0]) { //if the user was not found in the database (ie not saved to the redis database yet, look up the mongodb)
  ...
  io.on('connection', function(socket) {
    ...
    require('./../ux/socket-ux.js')(io,socket,RedisClient);
